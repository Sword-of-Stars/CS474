...
\begin{align*}
    ...
    {{variable.replace('S0', 'S_0').replace('[U', 'U_{').replace(']', '}')}} &\to {% for rule in rules -%}
    {% if rule == 'e' -%}
...
\begin{align*}
    ...
    {%- set formatted_var = variable.replace('S0', 'S_0').replace('[U', 'U_{').replace(']', '}') -%}
...
\begin{align*}
    ...
    {{variable.replace('S0', 'S_0').replace('[U', 'U_{').replace(']', '}')}} &\to {% for rule in rules -%}{{- "\\varepsilon" if rule == 'e' else rule -}}{% if not loop.last %} \mid {% endif %}{%- endfor -%}\\
    {% endfor -%}
\end{align*}
